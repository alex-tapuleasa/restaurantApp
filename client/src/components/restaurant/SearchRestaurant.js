...
function SearchRestaurants (props) {
    ...
    const searchRestaurants = async (searchQuery) => {
        toggleSearchDone();
        if(search.trim()) {
        const res = await axiosRender.get(`/restaurants/search?searchQuery=${search || 'none'}`);
        setUserContext(oldValues => {
            return { ...oldValues, searchRestaurants: res.data };
        })

        navigate(`/restaurants/search?searchQuery=${search || 'none'}`);
